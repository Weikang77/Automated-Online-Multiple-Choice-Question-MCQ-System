(function (w,$){
    ...
    function paper( target , option ){
        var options = {
            ...
            daan:'daan',
            leixing:'leixing',
            danxuanti:'Single topic selection',
    ...
    paper.prototype = {
        ...
        initKaoshishichang(){
            ...
            if(options.kaoshishichang > 0){
                ...
                function clock() {
                    ...
                    shichang.html('Remaining time：' + (shengyuH + "h" + shengyuM + "min" + S + "s" + "<br>"));
            ...
            for (var j=0;j<options.list.length;j++){
                ...
                if(options.danxuanti.indexOf(type)!==-1){
                    ...
                    if(op.length == 0 && options.isRequired && !force){
                        // 必填项
                        alert('Please choose【'+o[options.biaoti]+'】');
                        return;
                    }
                    if(op.length == 0){
                        obj.zimu = '';
                        ...
                        obj.zimu = op.val();
                        obj[options.daan] = op.data('title');
                        obj.defen = Math.floor(op.data('point'));
                    }
                    obj.success = obj.defen > 0 ? 1 : 0;
                    ...
                    obj[options.daan] = daan.join(",");
                    obj.defen = isCuo ? 0 : defen;
                    obj.zimu = zimu.join(",");
                    obj.success = successCount == op.length ? 1 : 0;
                    ...
                    obj[options.daan] = that.$form.find('[name="daan' + j + '"]').val();
                    obj.defen = -1;
                    obj.zimu = '';
                    obj.success = 1;
                }
        ...
        initButton(){
            return '<div style="margin-top: 15px"> <input type="submit" name="Submit" value="submit" class="btn btn-primary"/>\n' +
                '            <input type="reset" name="Submit2" value="reset" class="btn btn-default"/></div>';
        },
        initItems(){
            var that = this;
            ...
            $.each(options.list , function(i,o){
                ...
                result += '<div class="zhangjie-list" style="margin-bottom: 10px; padding: 10px; background: #fff">' +
                    ...
                    '<h3>Question' + (i + 1) + '：' + o[options.biaoti] + '</h3>' +
        ...
        createTextarea:function (j,obj){
            return '<textarea name="daan' + j + '" data-index="' + j +'" style="width: 100%;height: 85px;" placeholder="Please fill in the answer"></textarea>';
        }
    };
